...
class FedRunner(object):
    ...
    def __init__(self,
        ...
        assert self.cfg.federate.client_num >= self.cfg.federate.seen_client_num
        all_clients_id = np.arange(1, self.cfg.federate.client_num + 1).tolist()
        if self.cfg.federate.seen_client_num < 0:
            self.seen_clients_id = all_clients_id
            self.unseen_clients_id = []
        else:
            self.seen_clients_id = sorted(np.random.choice(all_clients_id,
                                                           size=self.cfg.federate.seen_client_num,
                                                           replace=False).tolist())
            self.unseen_clients_id = list(set(all_clients_id) - set(self.seen_clients_id))

        logger.info(
            f"Totally {self.cfg.federate.client_num} clients, only {self.cfg.federate.seen_client_num} clients are available, {self.cfg.federate.client_num-self.cfg.federate.seen_client_num} clients are unseen.")
        logger.info(f"#{len(self.seen_clients_id)} seen clients: {self.seen_clients_id}")
        logger.info(f"#{len(self.unseen_clients_id)} unseen clients: {self.unseen_clients_id}")
    ...
    def _setup_server(self, resource_info=None, client_resource_info=None):
        ...
        if self.server_class:
            ...
            server = self.server_class(
                ...
                seen_clients_id=self.seen_clients_id,
                **kw)
