...
class UniformSampler(Sampler):
    ...
    def sample(self, size, state):
        ...
        assert len(idle_clients) >= size, f"Not enough idle clients {len(idle_clients)}<{size}"
        sampled_clients = np.random.choice(idle_clients,
                                           size=size,
                                           replace=False).tolist()

        if state is not None:
            self.change_state(sampled_clients, state)
...
class GroupSampler(Sampler):
    ...
    def sample(self, size, state, shuffle=False):
        """
        To sample clients
        """
        if shuffle:
