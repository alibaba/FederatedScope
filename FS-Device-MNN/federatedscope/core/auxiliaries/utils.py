...
def merge_param_dict(raw_param, filtered_param):
    if not set(raw_param.keys()) == set(filtered_param.keys()):
        logger.info(f"Missing params within {list(set(raw_param.keys()) - set(filtered_param.keys()))}")

    for key in filtered_param.keys():
        if isinstance(filtered_param[key], np.ndarray):
            raw_param[key] = torch.from_numpy(filtered_param[key])
        else:
            raw_param[key] = filtered_param[key]
    return raw_param


def get_time_stamp(format="%Y-%m-%d %H:%M:%S"):
...
def print_progress_bar(n_cur, n_total, length=64):
    length_cur = int(n_cur / n_total * length)
    progress_bar = "[{}]".format(("#"*length_cur).ljust(length, " "))
    if n_cur >= n_total:
        end = "\n"
    else:
        end = "\r"
    print(f"{progress_bar} [{n_cur}/{n_total}]", end=end)
