...
class global_NT_xentloss(nn.Module):
    ...
    def forward(self, z1, z2, others_z2=[]):
        ...
        labels = torch.zeros(2*N, dtype=torch.int64, device=self.device) # scalar label per sample
...
#         torch.FloatTensor
#     """
#     z1, z2 = z1.cuda(device=device), z2.cuda(device=device)
#     N, Z = z1.shape 
#     representations = torch.cat([z1, z2], dim=0)
#     similarity_matrix = F.cosine_similarity(representations.unsqueeze(1), representations.unsqueeze(0), dim=-1)
...
#     if len(others_z2) != 0:
#         for z2_ in others_z2:
#             z2_ = z2_.cuda(device=device)
#             N2, Z2 = z2_.shape
#             representations = torch.cat([z1, z2_], dim=0)
#             similarity_matrix = F.cosine_similarity(representations.unsqueeze(1), representations.unsqueeze(0), dim=-1)
#             mask = torch.zeros_like(similarity_matrix, dtype=torch.bool, device=device)
#             mask[N:,:N] = True
#             mask[:N,N:] = True
#             negatives_other = similarity_matrix[mask].view(2*N, -1)
#             negatives = torch.cat([negatives, negatives_other], dim=1)
...
#     labels = torch.zeros(2*N, dtype=torch.int64, device=device) # scalar label per sample
