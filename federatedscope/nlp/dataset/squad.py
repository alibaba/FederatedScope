...
import torch
import logging
from federatedscope.nlp.dataset.utils import split_sent, DictDataset, NUM_DEBUG
...
def create_squad_examples(data, split, debug=False):
    if debug:
        data = data[:NUM_DEBUG]
    examples = []
    for para in data:
...
def create_squad_dataset(data,
                         ...
                         **kwargs):
    if pretrain:
        return create_squad_pretrain_dataset(data, split, tokenizer,
    ...
    if osp.exists(cache_file):
        ...
        examples = create_squad_examples(data, split, debug)
...
def create_squad_pretrain_dataset(data,
                                  split,
                                  tokenizer,
                                  max_seq_len,
                                  cache_dir='',
