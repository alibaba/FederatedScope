...
class PLServer(Server):
    def __init__(self,
        ...
        if self._cfg.federate.make_global_eval:
            ...
            self.best_client_val_res = -np.inf
    ...
    def broadcast_model_para(self,
        ...
        if not skip_broadcast:
            if self._cfg.federate.pl_init_kd and self.state == 0:
                ...
                # if not self.init_cfg.model.share_client_layer_param and \
                #         self._cfg.model.share_client_layer_param:
                #     num_layer = self._cfg.model.num_client_layers
                #     num_layer_per_cell = \
                #         self._cfg.model.num_client_layers_per_cell
                #     self.init_model.model.encoder.layer = \
                #         nn.ModuleList(
                #             list(self.init_model.model.encoder.layer[
                #             :num_layer_per_cell]) *
                #             (num_layer // num_layer_per_cell))
    ...
    def eval(self):
        if self._cfg.federate.make_global_eval:
            ...
            if self._cfg.federate.make_global_train:
                ...
                #     self.client_trainer.update_alter_stage('prompt')
                # train_metrics = self.client_trainer.train()[-1]
                # formatted_train_res = self._monitor.format_eval_res(
                #     train_metrics,
                #     rnd=self.state,
                #     role='Client #',
            ...
            if not self._cfg.federate.skip_local_train:
                ...
                cur_val_res = val_metrics[comp_metric]

                if cur_val_res > self.best_val_res:
                    self.best_val_res = cur_val_res
                    if self._cfg.federate.pl_save_to:
                        logger.info(f'Best score {cur_val_res} obtained. '
                ...
                # # client
                # val_metrics = self.client_trainer.evaluate(
                #     target_data_split_name='val')
                # formatted_val_res = self._monitor.format_eval_res(
                #     val_metrics,
                #     rnd=self.state,
                ...
                #     self._cfg.federate.pl_save_to, 'best_client_model.pt')
                # if cur_val_res > self.best_client_val_res:
                #     self.best_client_val_res = cur_val_res
                #     logger.info(f'Best client score {cur_val_res} obtained. '
                #                 f'Model saved to {client_save_path}.')
                #     ckpt = {
                #         'round': self.state,
                #         'val_res': cur_val_res,
                #         'model': self.client_trainer.get_model_para()
                #     }
                #     torch.save(ckpt, client_save_path)
                #
                # self.history_results = merge_dict_of_results(
                #     self.history_results, formatted_val_res)
                # self._monitor.save_formatted_results(formatted_val_res)
            ...
            if self.state == self.total_round_num:
                ...
                lm_eval_model = LMEvalModel(self.model, tokenizer)
                ...
                if os.path.exists(save_path):
                    ...
                    # # last ckpt (client)
                    # test_metrics = self.client_trainer.evaluate(
                    #     target_data_split_name='test')
                    # formatted_test_res = self._monitor.format_eval_res(
                    #     test_metrics,
                    #     rnd=self.state,
                    ...
                    #
                    # # best ckpt (client)
                    # if not self._cfg.federate.pl_only_test:
                    #     best_ckpt = torch.load(client_save_path,
                    #                            map_location='cpu')
                    #     self.client_trainer.update(best_ckpt['model'])
                    #     logger.info(f"Loaded best model obtained in round "
                    #                 f"{best_ckpt['round']} "
                    #                 f"({best_ckpt['val_res']}).")
                    #
                    ...
                    # shutil.rmtree(self._cfg.federate.pl_save_to)
