...
class PLDataProcessor(object):
    def __init__(self, config):
        ...
        if config.data.num_train_per_client != -1:
            self.num_train_samples = config.data.num_train_per_client * \
                                     self.num_clients
    ...
    def split_data(self):
        ...
        if self.non_iid_split:
            idx_slice = dirichlet_distribution_noniid_slice(
                label=labels[:self.num_train_samples],
                client_num=self.num_clients,
                alpha=0.5)
        else:
            indices = np.arange(0, len(labels))[:self.num_train_samples]
