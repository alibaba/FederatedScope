...
from federatedscope.nlp.loss.label_smooth_loss import LabelSmoothingLoss
    ...
    def forward(
        ...
        if self.task == 'pretrain':
            ...
            if label_smoothing > 0:
                ...
                loss = loss_fct(
                    F.log_softmax(
                        logits.contiguous().view(-1, self.vocab_size), dim=-1),
                ...
                if len(example_indices) > 0 and len(topk_group_ids) > 1:
                    ...
                    if len(neg_client_ids) > 0:
                        ...
                        dissim_matrix = F.cosine_similarity(
                            ...
                            dissim_hiddens,
                            dim=-1)
                        denominator = torch.exp(dissim_matrix /
                                                self.contrast_temp).sum(0)
                        contrast_loss = -torch.log(
                            torch.exp(nominator) / denominator).mean()
                    else:
