...
class FedNLPAggregator(ClientsAvgAggregator):
...
class PFedNLPAggregator(FedNLPAggregator):
    def __init__(self, model=None, config=None, device='cpu'):
        super().__init__(model=model, config=config, device=device)
...
class PCFedNLPAggregator(PFedNLPAggregator):
    def __init__(self, model=None, config=None, device='cpu'):
        super().__init__(model=model, config=config, device=device)
    ...
    def _compute_client_groups(self, models):
        ...
        if self.task == 'pretrain':
            ...
            distances = cosine_distances(grads, grads)
            proto = torch.stack([
                torch.stack(list(feat.values())).mean(0)
                for feat in self.contrast_monitor.dec_hidden.values()
            ])
            ...
            self.client_id2topk_proto = [
                dis[:k].tolist() for dis, k in zip(
                    np.argsort(dist_proto, axis=-1), self.num_agg_topk)
            ]
            self.client_id2all = np.argsort(distances, axis=-1).tolist()
        return tasks

    def _avg_params_with_proto(self, models, client_adj_norm,
                               client_adj_norm_proto):
        avg_model = copy.deepcopy([{
            n: p
            for n, p in model.state_dict().items()
            if n in models[0]['model_grads']
            ...
            for i in range(len(avg_model)):
                for j in range(len(avg_model)):
                    ...
                    if j == 0:
                        ...
                        avg_grads_proto[i][k] += local_grad * weight_proto
                avg_model[i][k] = \
                    avg_model[i][k].float() + (1 - self.proto_weight) * \
                    avg_grads[i][k] + self.proto_weight * avg_grads_proto[i][k]
    ...
    def _para_weighted_avg(self, models, recover_fun=None):
        ...
        if self.task == 'pretrain':
            ...
            if contrast_stat == 2:
                dec_hidden = [
                    model['contrast_monitor'].dec_hidden for model in models
                ]
                dec_out = [
                ...
                client_adj_proto = torch.zeros(self.client_num,
                                               self.client_num)
                for i in range(self.client_num):
                    for j in range(self.client_num):
                        if j in self.client_id2topk_proto[i]:
                            client_adj_proto[i][j] = \
                                models[j]['sample_size'] * self.inside_weight
                        else:
                            client_adj_proto[i][j] = \
                                models[j]['sample_size'] * self.outside_weight
                client_adj_norm_proto = \
                    client_adj_proto / client_adj_proto.sum(dim=-1,
                                                            keepdim=True)

                # aggregate model params
                model_params = {
                    'model_para': self._avg_params_with_proto(
                        models, client_adj_norm, client_adj_norm_proto)
                }
