...
class vFLServer(Server):
    ...
    def __init__(self,
        ...
        cfg_key_size = config.secure_vertical.key_size
        self.public_key, self.private_key = \
            abstract_paillier.generate_paillier_keypair(n_length=cfg_key_size)
        self.dims = [0] + config.secure_vertical.dims
        self.theta = self.model.state_dict()['fc.weight'].numpy().reshape(-1)
        self.lr = config.train.optimizer.lr

        self.register_handlers('encrypted_gradient',
                               self.callback_funcs_for_encrypted_gradient)
    ...
    def callback_funcs_for_encrypted_gradient(self, message: Message):
