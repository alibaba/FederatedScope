...
class Test_base:
    ...
    def evaluation(self):
        test_mean = [
            x / (self.client.state + 1) + 1e-7 for x in self.client.test_result
        ]
        if self.client.criterion_type == 'CrossEntropyLoss':
            test_result = [1 if x >= 0.5 else 0 for x in test_mean]
            loss = np.mean(-self.client.test_y * np.log(test_mean))
            acc = np.sum(test_result == self.client.test_y) / len(
                self.client.test_y)
            metrics = {
                'test_loss': loss,
                'test_acc': acc,
