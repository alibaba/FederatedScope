...
from federatedscope.vertical_fl.gbdt.worker.Feature_sort_base\
    import Feature_sort_base
from federatedscope.vertical_fl.gbdt.worker.Feature_sort_by_bin\
    import Feature_sort_by_bin
from federatedscope.vertical_fl.gbdt.worker.Test_base import Test_base

from federatedscope.vertical_fl.gbdt.worker.Loss_function \
    import TwoClassificationloss, Regression_by_mseloss, Regression_by_maeloss

logger = logging.getLogger(__name__)


class GBDTClient(Client):
    def __init__(self,
                 ID=-1,
                 server_id=None,
                 state=0,
    ...
    def cal_se(self, left, indicator, y):
        left = left * indicator
        right = indicator - left
        real_y = indicator * y
        left_y = left * real_y
        right_y = right * real_y
        if not left.any() or not right.any():
            return float('inf')
        else:
            left_mean = np.sum(left_y) / np.sum(left)
            right_mean = np.sum(right_y) / np.sum(right)
            err = np.sum((left_y - left * left_mean)**2) + np.sum(
                (right_y - right * right_mean)**2)
            return err
