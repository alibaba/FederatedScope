...
class FTSServer(Server):
    ...
    def __init__(self,
        ...
        self.rand_feat_path = os.path.join(self._cfg.hpo.working_folder,
        ...
        self.res = [{
            'max_value': None, 'max_param': None,
            'all_values': [], 'all_params': [],
            } for _ in range(N)]
        self.res_paths = [os.path.join(self._cfg.hpo.working_folder,
    ...
    def _sample_from_this(self, client, y_max, iteration, info_ts):
        ...
        x_max, all_ucb = acq_max(ac=self.util_ts.utility, gp=self.gp[client],
                                 M=M_target, N=self.num_other_clients,
                                 gp_samples=None,
    ...
    def _sample_from_others(self, client, y_max, iteration, info_ts):
        ...
        x_max, all_ucb = acq_max(ac=self.util_ts.utility, gp=self.gp[client],
                                 M=self.M, N=self.num_other_clients,
                                 gp_samples=None,
                                 random_features=self.random_feats,
                                 info_ts=info_ts, pt=self.pt,
                                 ws=self.ws, use_target_label=False,
                                 w_sample=w_sample, y_max=y_max,
                                 bounds=self.bounds, iteration=iteration)
        return x_max, all_ucb
