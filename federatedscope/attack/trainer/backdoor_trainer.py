...
def wrap_backdoorTrainer(
    ...
    if base_trainer.cfg.attack.self_opt:
        ...
        base_trainer.register_hook_in_train(new_hook=hook_on_fit_start_init_local_opt,
    ...
    if base_trainer.cfg.attack.scale_poisoning or base_trainer.cfg.attack.pgd_poisoning:
        ...
        base_trainer.register_hook_in_train(new_hook=hook_on_fit_start_init_local_model,
                                            trigger='on_fit_start',
                                            insert_pos=-1)
    ...
    if base_trainer.cfg.attack.scale_poisoning:
        ...
        base_trainer.register_hook_in_train(new_hook=hook_on_fit_end_scale_poisoning,
                                            trigger="on_fit_end",
                                            insert_pos=-1)
    ...
    if base_trainer.cfg.attack.pgd_poisoning:
        ...
        base_trainer.register_hook_in_train(new_hook=hook_on_fit_start_init_local_pgd,
                                        trigger='on_fit_start',
                                        insert_pos=-1)


        base_trainer.register_hook_in_train(new_hook=hook_on_batch_end_project_grad,
                                        trigger='on_batch_end',
                                        insert_pos=-1)



        base_trainer.register_hook_in_train(new_hook=hook_on_epoch_end_project_grad,
...
def hook_on_fit_start_init_local_opt(ctx):
    ...
    # ctx.optimizer = torch.optim.SGD(ctx.model.parameters(), lr=self.adv_lr*self.args_gamma**(flr-1), momentum=0.9, weight_decay=1e-4) # looks like adversary needs same lr to hide with others
    ctx.optimizer = torch.optim.SGD(ctx.model.parameters(), lr=ctx.self_lr, momentum=0.0, weight_decay=0) # looks like adversary needs same lr to hide with others
...
def hook_on_fit_end_scale_poisoning(ctx):
    ...
    logger.info("the Norm of the original global model: {}".format(torch.norm(v)))
    ...
    for idx, param in enumerate(ctx.model.parameters()):
        param.data = (param.data - ctx.original_model [idx])*scale_para + ctx.original_model[idx]
...
def hook_on_fit_start_init_local_pgd(ctx):
    ...
    ctx["num_train_epoch"] = ctx.self_epoch
    # ctx.optimizer = torch.optim.SGD(ctx.model.parameters(), lr=self.adv_lr*self.args_gamma**(flr-1), momentum=0.9, weight_decay=1e-4) # looks like adversary needs same lr to hide with others
    ctx.optimizer = torch.optim.SGD(ctx.model.parameters(), lr=ctx.pgd_lr, momentum=0.9, weight_decay=1e-4) # looks like adversary needs same lr to hide with others
...
def hook_on_batch_end_project_grad(ctx):
    ...
    model_original_vec = parameters_to_vector(list(ctx.original_model.parameters()))
    # make sure you project on last iteration otherwise, high LR pushes you really far
    if (ctx.batch_index % project_frequency == 0) and (torch.norm(w_vec - model_original_vec) > eps):
