...
class PassiveServer(Server):
    ...
    '''
    def __init__(self, ID=-1, state=0, data=None, model=None, client_num=5,
                 total_round_num=10, device='cpu', strategy=None,
                 state_to_reconstruct=None, client_to_reconstruct=None,
                 **kwargs):
        super(PassiveServer,
              self).__init__(ID=ID, state=state, data=data, model=model,
                             client_num=client_num,
                             total_round_num=total_round_num, device=device,
    ...
    def _get_reconstructor(self):
        ...
        return get_reconstructor(
            self.atk_method, max_ite=self._cfg.attack.max_ite,
            lr=self._cfg.attack.reconstruct_lr,
            federate_loss_fn=self.model_criterion, device=self.device,
    ...
    def _reconstruct(self, state, sender):
        ...
        dummy_data, dummy_label = self.reconstructor.reconstruct(
            ...
            data_feature_dim=self.data_dim, num_class=self.num_class,
                ...
                if self.reconstructed_data_sav_fn is not None:
                    self.reconstructed_data_sav_fn(
                        data=self.reconstruct_data[message.state][
                            message.sender][0], sav_pth=self._cfg.outdir,
...
class PassivePIAServer(Server):
    ...
    def __init__(self, ID=-1, state=0, data=None, model=None, client_num=5,
        ...
        self.pia_attacker = PassivePropertyInference(
            ...
            fl_model_criterion=get_criterion(self._cfg.criterion.type,
                                             device=self.device),
            device=self.device, grad_clip=self._cfg.grad.grad_clip,
            ...
            fl_lr=self._cfg.optimizer.lr, batch_size=100)
    ...
    def callback_funcs_model_para(self, message: Message):
        ...
        self.pia_attacker.collect_updates(
            ...
            updated_parameter=content[1], round=round, client_id=sender)
