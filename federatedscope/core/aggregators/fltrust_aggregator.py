...
class FLtrustAggregator(ClientsAvgAggregator):
    """
    Implementation of FLtrust refer to `FLTrust: Byzantine-robust Federated Learning \
        via Trust Bootstrapping`
    [Cao et al., 2020]
    (https://www.ndss-symposium.org/wp-content/uploads/ndss2021_6C-2_24434_paper.pdf)

    It computes the trustcore of each update based on a root dataset on server side.
    """
    ...
    def _aggre_with_fltrust(self, models, global_delta):
        ...
        for each_model in models:
            ...
            TS = torch.dot(tmp_delta,global_delta_temp)/(torch.linalg.norm(tmp_delta)*torch.linalg.norm(global_delta_temp))
            if TS < 0:
                TS = 0
            total_TS += TS
            norm = torch.linalg.norm(tmp_delta)/torch.linalg.norm(global_delta_temp)
