...
class Monitor(object):
    ...
    def format_eval_res(self,
        ...
        Returns:
            ...
            'Results_fairness': {
             'test_total': 33.99, 'test_correct': 27.185, 'test_avg_loss_std': 0.433551,
             'test_avg_loss_bottom_decile': 0.356503, 'test_avg_loss_top_decile': 1.212492,
             'test_avg_loss_min': 0.198317, 'test_avg_loss_max': 3.603567,
             'test_avg_loss_bottom10%': 0.276681, 'test_avg_loss_top10%': 1.686649,
             ...
             'test_acc_bottom_decile': 0.676471, 'test_acc_top_decile': 0.916667,
                ...
                for key in results.keys():
                    ...
                    if key in [
                        ...
                        if form == "fairness" and all_res.size > 1:
                            ...
                            #bottom and top 10%
                            new_results[f"{key}_bottom10%"]=np.mean(all_res[:
                                all_res.size  // 10])
                            new_results[f"{key}_top10%"]=np.mean(all_res[
                                all_res.size * 9 // 10:])
                            ...
                            new_results[f"{key}_entropy"]=np.sum(-all_res_preprocessed/np.sum(all_res_preprocessed)*(np.log((all_res_preprocessed)/np.sum(all_res_preprocessed))))
