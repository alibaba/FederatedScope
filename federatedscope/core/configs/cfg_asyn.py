...
def extend_asyn_cfg(cfg):
    ...
    cfg.asyn.aggregator = 'goal_achieved' # ['goal_achieved', 'time_up'] 'goal_achieved': perform aggregation when the defined number of feedback has been received; 'time_up': perform aggregation when the allocated time budget has been run out
    cfg.asyn.broadcast_manner = 'after_aggregating' # ['after_aggregating', 'after_receiving'] 'after_aggregating': broadcast the up-to-date global model after performing federated aggregation; 'after_receiving': broadcast the up-to-date global model after receiving the model update from clients
...
def assert_asyn_cfg(cfg):
    ...
    ), "The time budget (seconds) must be an int or a float value, but {} is got".format(
    ...
    assert cfg.asyn.staleness_toleration >= 0 and isinstance(cfg.asyn.staleness_toleration, int), f"Please provide a valid staleness toleration value, expect an integer value that is larger or equal to 0, but got {cfg.asyn.staleness_toleration}."


    assert cfg.asyn.aggregator in ["goal_achieved", "time_up"], \
        f"Please specify the cfg.asyn.aggregator as the string 'goal_achieved' or 'time_up'. But got {cfg.asyn.aggregator}."
    assert cfg.asyn.broadcast_manner in ["after_aggregating", "after_receiving"], \
        f"Please specify the cfg.asyn.broadcast_manner as the string 'after_aggregating' or 'after_receiving'. But got {cfg.asyn.broadcast_manner}."
